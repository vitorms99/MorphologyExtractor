...
class BackgroundEstimator:
    ...
    def background_from_config(self, config = None):
        """
        Estimate the background based on the configured method.
        ...
        bkg_method = config.get('bkg_method', "frame")
        
        if bkg_method == 'flat':
            value = config.get('value', 0)
            std = config.get('std', 1)
            return self.flat_background(value, std)
        
        elif bkg_method == 'frame':
            image_fraction = config.get('image_fraction', 0.1)
            sigma_clipping = config.get('sigma_clipping', True)
            clipping_threshold = config.get('clipping_threshold', 3)
            return self.frame_background(image_fraction, sigma_clipping, clipping_threshold)
            ...
            fw = config.get('fw', 3)
            fh = config.get('fh', 3)
            return self.sep_background(bw, bh, fw, fh)
        
        elif bkg_method == 'load':
            bkg_image_path = config.get("bkg_image_path", "")
            bkg_image_prefix = config.get("bkg_image_prefix", "")
            bkg_image_sufix = config.get("bkg_image_sufix", "")
            bkg_image_HDU = config.get("bkg_image_HDU", 0)
            return self.load_background(bkg_image_path, bkg_image_prefix, bkg_image_sufix, bkg_image_HDU)
