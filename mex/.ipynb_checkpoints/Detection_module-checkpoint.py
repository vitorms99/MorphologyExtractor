...
class ObjectDetector:
    ...
    def detection_from_config(self, config = None, bkg_std = 0):
        """
        ...
        detected_objects : pandas DataFrame
            Catalog of detected objects with consistent column names.
        segmentation_map : ndarray
            Segmentation map from the detection.
        """
        if config is None:
            raise NameError("Config is not provided")
        
        elif config is not None:
            if not isinstance(config, dict):
                raise TypeError("Config must be a dictionary.")

        detection_mode = config.get('detection_mode', "sep")
        
        if detection_mode == 'sex':
        
            sex_folder = config.get('sex_files_folder', './')
            sex_default = config.get('default_file', 'default.sex')
            sex_keywords = config.get('sex_keywords', None)
            sex_output_folder = config.get('sex_output_folder', './')
            clean_up = config.get('clean_sex_output', True)
            
            return self.sex_detector(sex_folder, sex_default, sex_keywords, sex_output_folder, clean_up)
        
        elif detection_mode == 'sep':
            thresh = config.get('thresh', 1.5)
            minarea = config.get('minarea', 10)
            deblend_nthresh = config.get('deblend_nthresh', 32)
            deblend_cont = config.get('deblend_cont', 0.005)
            filter_type = config.get('filter_type', "matched")
            return self.sep_detector(thresh = thresh, minarea = minarea, deblend_nthresh = deblend_nthresh,
                                      deblend_cont = deblend_cont, filter_type = filter_type, bkg_std = bkg_std, sub_bkg = False)
