...
class PetrosianCalculator:
    ...
    def rp_from_config(self, config = None):
        ...
        rp_thresh = config.get("rp_thresh", 0.2)
        aperture = config.get("aperture", "elliptical")
        optimize_rp = config.get("optimize_rp", True)
        interpolate_order = config.get("interpolate_order", 3)
        Naround = config.get("Naround", 3)
        rp_step = config.get("rp_step", 0.05)
        
        eta, growth_curve, raio, rp = self.calculate_petrosian_radius(rp_thresh, aperture, optimize_rp,
                                                                      interpolate_order, Naround, rp_step)
        
        return(eta, growth_curve, raio, rp)
